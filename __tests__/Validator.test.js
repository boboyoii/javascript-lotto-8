...
describe('구입 금액 검증', () => {
  test('숫자가 아니면 예외 발생', () => {
    expect(() => validator.validatePurchaseAmount(NaN)).toThrow(
...
describe('당첨 번호 검증', () => {
  test('입력한 번호가 6개가 아니면 예외 발생', () => {
    expect(() => validator.validateWinningNumbers([1, 2, 3, 4, 5])).toThrow(
      ERROR_MESSAGES.WINNING.INVALID_COUNT
    );
  });

  test('숫자가 아닌 값이 있으면 예외 발생', () => {
    expect(() =>
      validator.validateWinningNumbers([1, 2, NaN, 4, 5, 6])
  ...
  test('범위를 벗어난 숫자가 있으면 예외 발생', () => {
    expect(() => validator.validateWinningNumbers([1, 2, 3, 4, 5, 46])).toThrow(
      ERROR_MESSAGES.WINNING.OUT_OF_RANGE
    );
  });

  test('중복되는 숫자가 있으면 예외 발생', () => {
    expect(() => validator.validateWinningNumbers([1, 2, 3, 3, 5, 10])).toThrow(
      ERROR_MESSAGES.WINNING.HAS_DUPLICATE
    );
  });
...
describe('보너스 번호 검증', () => {
  ...
  test('숫자가 아니면 예외 발생', () => {
    expect(() => validator.validateBonusNumber(NaN, winningNumbers)).toThrow(
      ERROR_MESSAGES.BONUS.NOT_NUMBER
    );
  });

  test('1~45 범위를 벗어나면 예외 발생', () => {
    expect(() => validator.validateBonusNumber(50, winningNumbers)).toThrow(
      ERROR_MESSAGES.BONUS.OUT_OF_RANGE
    );
  });

  test('당첨 번호와 중복되면 예외 발생', () => {
    expect(() => validator.validateBonusNumber(3, winningNumbers)).toThrow(
      ERROR_MESSAGES.BONUS.DUPLICATE_WITH_WINNING
    );
  });

  test('정상 입력이면 통과', () => {
    expect(() =>
      validator.validateBonusNumber(10, winningNumbers)
    ).not.toThrow();
  });
});
